...
class Zoo
  ...
  def info_entry_fee(user)
    ...
    many = [@infant, @children, @adult, @senior]

    case user.age
    when 0..5
      puts "#{user.name}さんの入場料は#{many[0]}円です。"
    when 6..12
      puts "#{user.name}さんの入場料は#{many[1]}円です。"
    when 13..64
      puts "#{user.name}さんの入場料は#{many[2]}円です。"
    when 65..120
      puts "#{user.name}さんの入場料は#{many[3]}円です。"
    end
  end
end
